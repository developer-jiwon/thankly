...
import { Calendar, Search, Users, Bookmark, ChevronRight, ChevronLeft, LogOut } from 'lucide-react'
...
export function SideNav({ onNavItemClick, onLogout }: { onNavItemClick: (label: string) => void, onLogout: () => void }) {
  ...
  return (
    <motion.nav
      ...
      initial={{ width: '40px' }}
      animate={{ width: isExpanded ? '140px' : '40px' }}
          ...
          {navItems.map((item, index) => (
            ...
            >
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.span
                    className="ml-2 text-sm font-medium whitespace-nowrap text-gray-600 dark:text-gray-300"
